...
/**
 * Multiple Trees Test - Render two tree components to trigger DnD backend conflict
 */
export const MultipleTrees: Story = {
  args: {} as never,
  render: () => {
    ...
    const mockSlices = createMockSlices({ sha: 'mock-sha-123', allFiles: mockFiles });

    return (
      <div style={{ display: 'flex', height: '100vh', background: '#0a0a0a', gap: '16px', padding: '16px' }}>
        {/* First Panel */}
        <div style={{ flex: 1, border: '1px solid #333', borderRadius: '8px', overflow: 'hidden' }}>
          <div style={{ padding: '12px', background: '#1a1a1a', borderBottom: '1px solid #333', color: '#fff', fontSize: '14px', fontWeight: 'bold' }}>
            Panel 1
          </div>
          <MockPanelProvider
            contextOverrides={{
...
/**
 * Refresh Button Test - Test the refresh functionality and DnD backend lifecycle
 * Click the refresh button in the panel to test if it properly handles component remounting
 */
export const RefreshButtonTest: Story = {
  args: {} as never,
  render: () => {
    ...
    // Recreate slices on each fileTreeKey change
    const mockSlices = React.useMemo(() => {
      const sha = `mock-sha-${fileTreeKey}`;
      console.log('[RefreshButtonTest] Creating new slices with SHA:', sha);
      ...
      return new Map([
        [
          'fileTree',
          {
            ...
            error: null,
            refresh: async () => {
              console.log('[RefreshButtonTest] fileTree.refresh() called');
              setRefreshCount(prev => prev + 1);
              // Add a small delay to potentially catch overlapping renders
    ...
    // Mock events that listen for canvas:refresh and trigger slice refresh
    const mockEvents = React.useMemo(() => ({
      emit: (event: PanelEvent<unknown>) => {
        console.log('[RefreshButtonTest] Event emitted:', event.type);
        if (event.type === 'canvas:refresh') {
          console.log('[RefreshButtonTest] Triggering refresh...');
          setRefreshCount(prev => prev + 1);

          if (isDelayedRefresh) {
            // Delayed refresh to try to overlap renders
            setTimeout(() => {
              setFileTreeKey(prev => prev + 1);
            }, 5);
          } else {
    ...
    return (
      <div style={{ display: 'flex', height: '100vh', background: '#0a0a0a' }}>
        {/* Panel - no key so it re-renders without unmounting */}
        <div style={{ flex: 1 }}>
          <MockPanelProvider
            contextOverrides={{
              slices: mockSlices,
        ...
        >
          <div>
            ...
            <p style={{ margin: 0, fontSize: 12, color: '#aaa', lineHeight: 1.5 }}>
              Click the refresh button in the panel to test the DnD backend lifecycle.
            </p>
          </div>

          <div style={{ fontSize: 11, color: '#666', lineHeight: 1.5, background: '#0a0a0a', padding: 12, borderRadius: 6 }}>
            <strong style={{ color: '#aaa' }}>Current State:</strong>
            <div>SHA: mock-sha-{fileTreeKey}</div>
            <div>Refresh Count: {refreshCount}</div>
            <div>Delayed Refresh: {isDelayedRefresh ? 'ON' : 'OFF'}</div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <button
              onClick={() => setIsDelayedRefresh(prev => !prev)}
              style={{
                background: isDelayedRefresh ? '#dc2626' : '#16a34a',
                color: 'white',
                border: 'none',
                padding: '10px 14px',
                borderRadius: 6,
            ...
            >
              Toggle Delayed Refresh ({isDelayedRefresh ? 'ON' : 'OFF'})
            </button>

            <button
              onClick={() => {
                // Rapid refresh to try to trigger overlap
                for (let i = 0; i < 3; i++) {
                  setTimeout(() => {
                    setFileTreeKey(prev => prev + 1);
                  }, i * 50);
          ...
          <div style={{ fontSize: 11, color: '#22c55e', lineHeight: 1.5, background: '#0a0a0a', padding: 12, borderRadius: 6 }}>
            <strong style={{ color: '#aaa' }}>Instructions:</strong>
            <ol style={{ margin: '8px 0 0 0', paddingLeft: 16 }}>
              <li>Click the refresh button (🔄) in the panel header</li>
              <li>Or try the "Rapid Refresh" button below</li>
              <li>Watch the console for errors</li>
              <li>If you see "Cannot have two HTML5 backends" error, the bug is reproduced</li>
            </ol>
          </div>
        </div>
      </div>
