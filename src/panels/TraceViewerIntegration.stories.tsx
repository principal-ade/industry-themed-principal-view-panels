...
/**
 * State for workflow panel selection
 */
...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'Integration story showing TraceListPanel, TraceDetailsPanel, and WorkflowScenariosPanel working together.\n\n' +
          '**Interactions:**\n' +
          '- Click trace ID or unmatched span → Shows TraceDetailsPanel\n' +
          '- Click matched span (green) → Shows WorkflowScenariosPanel\n\n' +
          '**Mock Traces:**\n' +
          '- GET /api/users, POST /api/orders (unmatched)\n' +
          '- User Login Flow, Checkout Process (scenario-matched)\n' +
          '- Complete User Journey (multi-workflow)',
      },
    },
  },
  tags: ['autodocs'],
...
interface PanelInnerProps {
  ...
  workflowState: WorkflowPanelState | null;
  setWorkflowState: (state: WorkflowPanelState | null) => void;
}
  ...
  useEffect(() => {
    ...
    const unsubscribe = props.events.on('trace:selected', (event: PanelEvent) => {
      ...
      if (payload.trace) {
        ...
        setWorkflowState(null); // Clear workflow state when selecting a trace
      }
    });

    return unsubscribe;
  }, [props.events, setSelectedTrace, setWorkflowState]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = props.events.on('custom', (event: PanelEvent) => {
      ...
      if (payload.action === 'openCanvas') {
        ...
        // Use defaults for canvas data since we don't have versionSnapshots in this story
        setWorkflowState({
          canvasId: payload.canvasId || payload.canvas?.id || 'e-commerce',
        ...
        setSelectedTrace(null); // Clear trace selection when opening workflow
      }
    });

    return unsubscribe;
  }, [props.events, setWorkflowState, setSelectedTrace]);

  // Determine which panel to show on the right
  const rightPanel = workflowState ? (
    <WorkflowScenariosPanel
      {...props}
      selectedCanvasId={workflowState.canvasId}
      canvasPath={workflowState.canvasPath}
      canvasName={workflowState.canvasName}
      selectedWorkflowId={workflowState.workflowId}
      workflowPath={workflowState.workflowPath}
      workflowTemplate={workflowState.workflowTemplate}
      selectedTraceId={workflowState.traceId}
      highlightedSpanId={workflowState.spanId}
      selectedScenarioIdProp={workflowState.scenarioId}
    />
  ) : (
...
const TraceViewerWrapper: React.FC<{ direction: 'horizontal' | 'vertical' }> = ({ direction }) => {
  ...
  const [workflowState, setWorkflowState] = useState<WorkflowPanelState | null>(null);
